...
interface PurchaseReturnsCreateProps {
    purchase: PurchaseReturnCreatePurchase;
}
...
export default function PurchaseReturnsCreate({ purchase }: PurchaseReturnsCreateProps) {
    const form = useForm({
        purchase_id: purchase.id,
        return_date: new Date().toISOString().slice(0, 10),
        reason: '',
        quantities: Object.fromEntries(purchase.items.map((item) => [item.id, 0])) as Record<number, number>,
    });

    const setQuantity = (purchaseItemId: number, value: number) => {
        form.setData('quantities', { ...form.data.quantities, [purchaseItemId]: value });
    };
    ...
    const submit: FormEventHandler = (e) => {
        ...
        form.transform((data) => ({
            ...
            items: purchase.items
                ...
                .map((item) => ({ purchase_item_id: item.id, quantity: data.quantities[item.id] })),
        }));
