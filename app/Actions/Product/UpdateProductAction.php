...
class UpdateProductAction
{
    ...
    public function execute(Product $product, array $data): Product
    {
        return DB::transaction(function () use ($product, $data) {
            ...
            // Only reachable while the product still has zero movements —
            // guarded server-side by OpeningStockEditable on the request.
            $openingStock = (float) ($data['opening_stock'] ?? 0);

            if ($product->canSetOpeningStock() && $openingStock !== 0.0) {
