...
- [x] **8.1** `sales_orders`, `sales_order_items` migration+model — design doc-এর মিনিমাল schema (`order_no`, `order_date`, `expected_delivery_date`, `status`, `total_amount`, `advance_paid`) অনুযায়ী; `sales` টেবিলে `sales_order_id` (nullable FK, retrofit migration) যোগ করা হয়েছে যাতে fulfillment-এর real Sale-টা কোন order থেকে এসেছে ট্র্যাক করা যায়। `settings`-এ `sales_order_prefix`/`sales_order_next_number` (invoice/purchase numbering-এর মতোই প্যাটার্ন) + `generateSalesOrderNumber()`। নতুন `SalesOrderStatus` enum (pending/partial/completed/cancelled) — pending=advance ছাড়া, partial=advance নেওয়া হয়েছে কিন্তু fulfill হয়নি, completed=Sale-এ convert হয়ে গেছে। Chart of Accounts-এ নতুন **2150 Customer Advances** (liability) যোগ করা হয়েছে — design doc-এ এই নির্দিষ্ট account কোনটা হবে বলা ছিল না, তাই এইটা যোগ করা হলো (2100 Accounts Payable-এর পাশে, generic 2300 Other Liabilities-এর বদলে, যেহেতু এইটা একটা distinct trade-liability)। `CreateSalesOrderAction` (advance অপশনাল, নিলে AccountService+LedgerService+**JournalService** সবগুলো দিয়ে Dr {account}/Cr 2150 পোস্ট হয় — CLAUDE.md-এর "প্রতিটা টাকা-সংক্রান্ত Action journal posting ছাড়া সম্পূর্ণ না" রুল মেনে, design doc নিজে zaman journal posting আসার আগে লেখা হয়েছিল বলে সেটা যোগ করে দেওয়া হয়েছে)
- [x] **8.2** `ConvertSalesOrderToSaleAction` — CreateSaleAction+ConfirmSaleAction-ই reuse করে (নতুন করে stock/ledger/account লজিক লেখা হয়নি) — stock/ledger/account সবসময় ConfirmSaleAction দিয়েই move করে, ঠিক normal sale-এর মতোই। Order-এ নেওয়া advance দ্বিতীয়বার record হয় না: `Sale::recalculatePaymentTotals()`-এ এখন sale-টার `sales_order_id` থাকলে সেই sales_order-এর নামে রেকর্ড করা advance-টাও (এখনো `reference_type = 'sales_order'`-ই আছে, rewrite করা হয়নি) যোগ করে paid_amount বের করে — এইভাবেই design doc-এর "advance_paid already collected is carried into the Sale's paid_amount, so it isn't double-counted" কথাটা সত্যি হয়, cash কোথাও দ্বিতীয়বার touch না করেই। Idempotent: status already completed হলে existing sale-ই ফেরত দেয়, দ্বিতীয় Sale তৈরি হয় না (feature test দিয়ে যাচাই করা হয়েছে)
- [x] **8.3** Sales Order List + Add Page — List page-এ date/customer/status filter (Sales List-এর প্যাটার্নেই); Add page-এ ProductSearchInput দিয়ে item add, AccountPaymentRows দিয়ে optional advance; নতুন Show page-ও বানানো হয়েছে (টাস্কে explicit না থাকলেও "Convert to Sale" অ্যাকশন কোথাও থেকে trigger করা দরকার ছিল) — pending/partial অবস্থায় "Convert to Sale" বাটন একটা modal খোলে (অতিরিক্ত payment optional) এবং completed হলে linked Sale-এ যাওয়ার লিংক দেখায়। Sidebar-এ Sales group-এর নিচে "Sales Order" সাব-আইটেম যোগ করা হয়েছে

✅ সব ৩টা task শেষ — Feature test (`SalesOrderTest.php`, ৫টা: no-advance creation, advance journal+ledger, conversion carries the advance, idempotent re-conversion, pages render) সহ পুরো test suite (Sale/SaleReturn/JournalPosting-সহ) আবার pass করেছে, কোনো regression নেই।
